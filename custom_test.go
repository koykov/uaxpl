...
func TestCustomParse(t *testing.T) {
	testDS := func(filename string) error {
		...
		for i := 0; i < len(ds); i++ {
			...
			if ct := ctx.GetClientType(); ct.String() != stage.Client.Type {
			...
			if dt := ctx.GetDeviceType(); dt.String() != stage.Device.Type {
			...
			if o := ctx.GetOS(); o != stage.Device.OS {
				buf = append(buf, fmt.Errorf("OS mismatch: need '%s' got '%s'", stage.Device.OS, o))
			}
			if ov := ctx.GetOSVersionString(); ov != stage.Device.OSVersion {
